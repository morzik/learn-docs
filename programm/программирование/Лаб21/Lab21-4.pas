...
procedure srvozrast (y:A; var q:real);
var i,k,s,m:integer;
begin
k:=2018;
s:=0;
...
procedure dnimes (t:integer; y:a);
var i,j,k:integer;
begin
k:=0;
writeln('Колличество числа сотрудников, родившихся по дням месяца');
for i:=1 to 31 do
    begin
    for j:=1 to t do
    if y[j].pas.d.day=i then inc(k);
if k<>0 then writeln('В ',i,' числе месяца ',k);
k:=0;
end;
end;
