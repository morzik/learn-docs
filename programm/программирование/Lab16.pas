...
{-------------описание величин---------------------}
var Y,X:A;i,j,k,l,n,mk:integer;
...
{---------------------------------------------------------}
procedure neighbourhood(n:integer; h:A; var t:integer; var g:A);
...
for j:=1 to n do
  for i:=j+1 to n do begin
    if ((h[j].pas.ad.town=h[i].pas.ad.town) and (h[j].pas.ad.street=h[i].pas.ad.street) and (h[j].pas.ad.hause=h[i].pas.ad.hause)) then begin
      t:=t+1;
      f:=true;
      g[t]:=h[i];
    end;
  end;
if f=true then writeln('тут есть соседи') else writeln('нет соседей');
end;
